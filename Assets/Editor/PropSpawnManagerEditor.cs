...
[CustomEditor(typeof(PropSpawnerManager))]
public class PropSpawnerManagerEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        DrawPropertiesExcluding(serializedObject, "fixedProps");

        var fixedPropsProp = serializedObject.FindProperty("fixedProps");
        // Group by section and subsection
        var sectionGroups = new Dictionary<string, Dictionary<string, List<int>>>();

        for (int i = 0; i < fixedPropsProp.arraySize; i++)
        {
            var element = fixedPropsProp.GetArrayElementAtIndex(i);
        ...
        EditorGUILayout.LabelField("Fixed Props", EditorStyles.boldLabel);
            ...
            if (sectionFoldouts[sectionKvp.Key])
            {
                ...
                foreach (var subSectionKvp in sectionKvp.Value)
                {
                    ...
                    if (subSectionFoldouts[subKey])
                    {
                        ...
                        {
                            EditorGUILayout.PropertyField(fixedPropsProp.GetArrayElementAtIndex(idx), new GUIContent($"Prop {idx}"), true);
                        }
                        EditorGUI.indentLevel--;
                    }
                }
